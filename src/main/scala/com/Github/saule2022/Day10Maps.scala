...
object Day10Maps extends App {
  ...
  //  //you could even create a Map of integers mapping to strings, which is kind of similar to indexed sequence,
  ...
  println(myFavorites.get("car"))


  //so we could use if to check for key existence
  //  val myKey = "food" // this could be coming in from readLine or outside
  val myKey = "car" // this could be coming in from readLine or outside
  ...
  val myKeys = myFavorites.keys.toArray //toSeq //keys actually is a Set which we have not covered
  val myKeys2 = myFavorites.keys.toSet
  println(s"My keys "+ myKeys.mkString(","))
  println(s"My keys  $myKeys2" )
  println(s"My values $myValues")
  println(s"My values $myValuesToo")
  //
  //
  //
  //  //I can filter values out of one Map into another
  val greenMap = for ((key, value) <- myFavorites if value == "green") yield (key, value) //.toString()
  ...
  val mySeq = Seq(("one", 10), ("two", 20), ("three", 30), ("two", 2020), ("two", 220003))
  println(mySeq(1)) //will print ("two",20)
  val myMap = mySeq.toMap //so here the first two will be gone and overwritten by 3nd "two", the last one
  println(mySeq)
  println(myMap)
  //  //mapping together key with an index
