...
class Animal(val name:String,
  ...
  //an alternative to match syntax we could have used a Map structure with keys being animal Types and values being sounds
  ...
  def meet(contact:String): Unit = {
    if (likes(contact)) { //so if the contact is in the Set then this will be true
      println(s"Hello there, I like that \u263a \n ") // ☺
    }
